...
contract CounterTest is Test {
    ...
    function test_Increment() public {
        ...
        (uint8 v1, bytes32 r1, bytes32 s1) = vm.sign(owner1Key, userOpHash1.toEthSignedMessageHash());
        userOp1.signature = abi.encodePacked(r1, s1, v1);

        // sign this user operation with the owner, otherwise it will revert due to the singleowner validation
        bytes32 userOpHash2 = entryPoint.getUserOpHash(userOp2);
        (uint8 v2, bytes32 r2, bytes32 s2) = vm.sign(owner1Key, userOpHash2.toEthSignedMessageHash());
